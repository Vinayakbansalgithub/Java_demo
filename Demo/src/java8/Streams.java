...
public static void main(String[] args) {
	...
	List<Integer> uniqueStd=Stream.of(s1,s2,s3,s4,s5).map(obj->obj.getStd()).distinct().collect(Collectors.toList());
...
Integer[] std = { 9,10 };
    ...
    List<List<Student>> employeesInAllStd = Stream.of(std)
      .map(a->Student.getByStd(a))
    ...
    List<Student> studentFlatStream = employeesInAllStd.stream()
...
List<String> sortedStudent=Stream.of(s1,s2,s3,s4,s5)
     .map(Student::getName)
    .sorted()
     .collect(Collectors.toList());
...
System.out.println("--foreach without assign--");
	...
	// set some value for each element
		List<Student> ninethStudents= Stream.of(s1,s2,s3,s4,s5).filter(obj->obj.getStd()==9).collect(Collectors.toList());
		...
		// We saw how we used collect() to get data out of the stream. If we need to get an array out of the stream, 
