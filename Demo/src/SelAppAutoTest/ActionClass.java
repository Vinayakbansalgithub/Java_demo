...
public class ActionClass {
	...
	public static void AppiumScrollTo(AndroidDriver driver)
		 ...
		 * LogSupport.INFOPass(msg); ExtentHTML.pass(msg);
		 */
	...
	public static void BaseUri() throws IOException, InvocationTargetException,
		...
		public static void GetRequest() throws IOException, InvocationTargetException, NoSuchMethodException,
				SecurityException, IllegalAccessException, IllegalArgumentException, InterruptedException {
			if (BuildObjrep.TestCaseHashmap.get("Parameter") != null
					&& !BuildObjrep.TestCaseHashmap.get("Parameter").equals("")) {
				parameters = BuildObjrep.TestCaseHashmap.get("Parameter").split(",");
			...
			if (totalParameters != null && totalParameters != 0) {
				...
				case 1:
					// Statements
						...
						res = given()
							...
							.param(key[0], value[0]).when().get(BuildObjrep.objKeyValue
									.get(BuildObjrep.TestCaseHashmap.get("Elementlocation")));
				...
				case 2:
					...
					res = given()
							...
							.when().get(BuildObjrep.objKeyValue
									.get(BuildObjrep.TestCaseHashmap.get("Elementlocation")));
				...
				case 3:
					...
					res = given()
							...
							.param(key[0], value[0]).param(key[1], value[1]).param(key[2], value[2]).when()
				...
				case 4:
					...
					res = given()
							...
							.param(key[0], value[0]).param(key[1], value[1]).param(key[2], value[2]).param(key[3], value[3])
			...
			msg = "get request  "
					+ BuildObjrep.objKeyValue.get(BuildObjrep.TestCaseHashmap.get("Elementlocation"));
		...
		public static void statusCode() {
			...
			int requiredStatusCode = Integer.parseInt(BuildObjrep.TestCaseHashmap.get("Data"));
			if (res.statusCode() == requiredStatusCode) {

				msg = "verify :  status code form API is " + res.statusCode() + "  is verified against "
						+ BuildObjrep.TestCaseHashmap.get("Data") + " and are same ";
			...
			if (res.statusCode() != requiredStatusCode) {
				...
				msg = "verify :  status code form API " + res.statusCode() + "  and required status code "
		...
		/*public static void VerifyAttribute() throws JSONException {
			JSONObject obj = new JSONObject(responseBody);
			flag = false;
			sheetValue = BuildObjrep.TestCaseHashmap.get("Data");
			Iterator itr = obj.keys();
			while (itr.hasNext()) {
				// Get key somehow? itr.getKey() ???
			...
			for (int i = 0; i < listAttributes.size(); i++) {
				...
				if (listAttributes.get(i).equalsIgnoreCase(sheetValue)) {
					msg = "Attribute  " + sheetValue + "   found in the list";
			...
			if (flag == false) {
				...
				msg = "value " + sheetValue + "  not  found in the list";
...
*/
		public static void VerifyValue() throws InterruptedException {
			sheetValue = BuildObjrep.TestCaseHashmap.get("Data");
			sheetValue = sheetValue.trim();
			flag = false;
			// System.out.println("list valus are");
			for (int i = 0; i < listValues.size(); i++) {

				// System.out.println("value " + i + " " + listValues.get(i));
				if (listValues.get(i).equalsIgnoreCase(sheetValue)) {
					msg = "value " + sheetValue + "   found in the list";
		...
		public static void close() throws InterruptedException {
			...
			BuildObjrep.emailBody.append("<td>" + BuildObjrep.TestSuitHashmap.get("Testcaseid") + "</td>");
			BuildObjrep.emailBody.append("<td>" + ExtentHTML.testrep.getRunStatus().toString() + "</td>");
			BuildObjrep.emailBody.append("<td>" + BuildObjrep.TestSuitHashmap.get("Environment") + "</td>");
			BuildObjrep.emailBody.append("</tr>");*/

			LogSupport.INFOPass(msg);
			ExtentHTML.pass(msg);
			System.out.println("test case status is " + ExtentHTML.testrep.getRunStatus().toString());
			System.out.println("test case enviroment is " + BuildObjrep.TestSuitHashmap.get("Environment"));

			System.out.println("test case name is " + BuildObjrep.TestSuitHashmap.get("Testcaseid"));
