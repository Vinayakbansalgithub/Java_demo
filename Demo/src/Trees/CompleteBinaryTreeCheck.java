...
static int i=1;
static boolean checkComplete(Comnode parent) {
	 
	 Comnode temp=parent;
	// TODO Auto-generated method stub
	 i++;
	 if (temp.left == null && temp.right==null && i<=h-2 ){
	 ...
	 while(i<=h-2)
	 {
		 ...
		 checkComplete(temp.left);
		 checkComplete(temp.right);
...
static int k=0;
static boolean leftnode=false;
static boolean rightnode=false;
static boolean move=true;
 static boolean leafCheck(Comnode parent) {
		// TODO Auto-generated method stub
	 Comnode temp=parent;
	 if (temp == null)
			return false;
	...
	if(temp.left!=null && move==true){
		
		leftnode=true;
	}else{
		leftnode=false;
	}
if( temp.right!=null  && move==true){
		if(leftnode)
rightnode=true;
		else
			return move=false;
